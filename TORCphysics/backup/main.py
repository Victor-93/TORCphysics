...
parser.add_argument("-ig", "--input_genome", action="store", help="Genome input file", default="sites_convergent.csv")
...
for i in range(N):
    ...
    #Should I check if RNAP is at the edge?
...
#For the genome
genome_df['fun'] = '' #This will be erased at the end
for i in range(n_genome):
...
print(o_df)
#AQUIMEQUEDE
#No olvides que tienes que quitar los fun al final
sys.exit()
...
if continuation:
    ...
    if N > 0: #This can only happen if there are objects bound to the DNA
        ...
        s_0 = genome_df.iloc[0]['superhelical']
        s_N = 0#genome_df.iloc[0]['superhelical'] #we don't need the ends on the right (Twist or superhelical)
        t_0 = genome_df.iloc[0]['twist']
        t_N = 0#genome_df.iloc[0]['twist']


    #Let's treat the boundaries of our system as objects.
    #----------------------------------------------------------------
    #Notice that we don't specify the end
    extra = { 'type': ['EXT', 'EXT'], 'name':['EXT_L', 'EXT_R'], 'start': [start00, startNN], 
            'end':[0,0], 'direction':[0,0], 'superhelical':[s_0,s_N], 'min_rate':[0,0],
            'max_rate':[0,0], 'twist':[t_0,t_N] }
    extra = pd.DataFrame(data=extra)
    #WARNING!!!!
...
bind_rate = np.zeros( n_genes ) #this measures the binding times (to measure rate)
nb    = np.zeros( n_genes ) #to calculate the number of bound proteins

output_array = np.zeros(( frames, n_genes, 5 )) #this counts 
...
aux_array = np.zeros( (n_genes,5) ) #This one is an auxiliar of the one above but it is an auxiliar to fill it
...
for k in range(frames): #Go by frame 
    ...
    #--------------------------------------------------------------------------
    #--------------------------------------------------------------------------
    for i in range(n_genes): #Let's go gene by gene
       ...
       #     break
        ...
        #-------------------------------------------------------------
        start_i = genes_df.iloc[i]['start']        #the start site
        end_i   = genes_df.iloc[i]['end']          #the end site
        d_i     = genes_df.iloc[i]['direction']    #the direction
        ...
        if d_i == 1:
            d = o_df.iloc[idx+1]['start'] - start_i #distance between promoter and RNAP on the right
        elif d_i == -1:
            d = start_i - o_df.iloc[idx]['start']  #distance between promoter and RNAP on the left
        else:
        ...
        if d <= params.OBJECT_size[ o_df.iloc[idx+1]['name'] ]:
        ...
        if genes_df.iloc[i]['name'] == 'tetA': #I way to make tetA with constant rate
            prob = sm.P_binding_Poisson( genes_df.iloc[i]['min_rate'] ) #should be the same as max_rate
            #print('tetA')
        else:
            prob = sm.P_binding_Nonh_Poisson([genes_df.iloc[i]['min_rate'],genes_df.iloc[i]['max_rate']],
                                              o_df.iloc[idx]['superhelical'], f_ini )
            #print('not tetA')
            #
        ...
        if uran <= prob:           #and decide
            ...
            #RNAP + its direction
            extra = { 'type': ['RNAP'], 'name':['RNAP'], 'start': [start_i], 
                      'end':[end_i], 'direction':[d_i], 'superhelical':[0], 'rate':[0], 'twist':[0] }
            extra = pd.DataFrame(data=extra)
            ...
            twist_l = T_twist*(  ( length_l + 0.5*params.OBJECT_size[extra.iloc[0]['name']] )/T_length  )
            twist_r = T_twist*(  ( length_r + 0.5*params.OBJECT_size[extra.iloc[0]['name']] )/T_length  )
    ...
    for i in range(1,N-1):    #Let's go through each object, updating its twist and
        ...
        if o_df.iloc[i]['direction'] != 0:
            ...
            #Object i MOVES!
            #-----------------
            o_df.at[i,'start'] = mm.RNAP_motion( o_df.iloc[i] )
    ...
    for j in range(1,N-1):    #We need to go through all objects again    
        ...
        if ( o_df.iloc[i]['direction'] == 1  and o_df.iloc[i]['end'] - o_df.iloc[i]['start'] <= 0 ) or \
           ( o_df.iloc[i]['direction'] == -1 and o_df.iloc[i]['end'] - o_df.iloc[i]['start'] >= 0 ):
            ...
            mask = genes_df['end'] == o_df.iloc[i]['end']
            a = genes_df[mask].index
...
#Genome will now contain the average supercoiling and twist.
print(N)
if circular:
    if N > 2:
        genome_df.at[0, 'superhelical'] = (o_df['twist'].sum() - o_df.iloc[0]['twist'])/(w0*nbp)
        genome_df.at[0, 'twist'] = (o_df['twist'].sum() - o_df.iloc[0]['twist'])
    else:
        genome_df.at[0, 'superhelical'] = o_df['twist'].sum()/(w0*nbp)
        genome_df.at[0, 'twist'] = o_df['twist'].sum()
else: #linear
    genome_df.at[0, 'superhelical'] = o_df['twist'].sum()/(w0*nbp)
    genome_df.at[0, 'twist'] = o_df['twist'].sum()

#concatenate the dataframes into one (out_df) and print it
out_df = pd.concat( [genome_df, genes_df, o_df]  )
out_df.to_csv(coutput, index=False, sep='\t')
