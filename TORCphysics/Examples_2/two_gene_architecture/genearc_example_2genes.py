...
# TODO: A function similar to this one needs to be created for two genes
...
def make_2gene_site_csv(filename, stypes, names, starts, ends, k_ons, bmodels, paramsfiles, ngenes):
    info_list = []
    for n in range(ngenes):
        info = {
            'type': stypes[n], 'name': names[n], 'start': starts[n], 'end': ends[n], 'k_on': k_ons[n],
            'binding_model': bmodels[n], 'binding_oparams': paramsfiles[n]
        }
        info_list.append(info)
    site_df = pd.DataFrame(info_list)
    site_df.to_csv(filename, index=False)
...
circuit_size = upstream_distance + gene_length + 500 + gene_length + downstream_distance
...
# Gene 2
start2 = upstream_distance + gene_length + 500
end2 = upstream_distance + gene_length + 500 + gene_length
#make_gene_site_csv(sites_filename, 'gene', promoter_case+'_2', start2, end2, 1,
#                   reporter_binding_model_name, 'none')

make_2gene_site_csv(sites_filename, stypes=['gene', 'gene'], names=[promoter_case+'_1', promoter_case+'_2'],
                    starts=[start, start2], ends=[end, end2], k_ons=[0.1, 0.5],
                    bmodels=[reporter_binding_model_name, reporter_binding_model_name], paramsfiles=['none', 'none'],
                    ngenes=2)
