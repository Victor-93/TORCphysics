#import sys
#sys.path.extend(['/nobackup/phyvve/github_code/'])
...
# DESCRIPTION
# ----------------------------------------------------------------------------------------------------------------------
# The idea is to calibrate topo_k_cat using the hyperopt library and many cpus from ARC3

# Get the number of available CPUs
num_cpus = multiprocessing.cpu_count()

# Adjust the number of workers based on the available CPUs
num_workers = num_cpus  # Set it to num_cpus or adjust as per your requirements
chunksize = 5  # 5 sims per worker?
nsim = num_workers * chunksize  # in total


# Set up a local cluster with the desired number of workers
cluster = LocalCluster(n_workers=num_workers)
client = Client(cluster)

print(num_workers, chunksize, nsim)

# ----------------------------------------------------------------------------------------------------------------------
# INITIAL CONDITIONS
# ----------------------------------------------------------------------------------------------------------------------
ntests = 2#200  # number of tests for parametrization
...
# Shutdown the cluster and close the client
client.shutdown()
client.close()