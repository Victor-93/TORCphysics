...
promoter_selection = 'both'

# If True, then the minimum and full promoters will have different parameterisations!
full_minimum_different = False
full_minimum_different = True
...
n_simulations = 2#2#24 #180 - ask for 61 cores (Each would run 3 simulations per system approximately).
n_batches = 2#2 # The number of simulations n_simulations will be grouped into n_batches. For each batch, an average
...
file_out = 'dist_op_TORC_plasmid_st3.4_test'
...
# Quick check!
if promoter_selection == 'full':
    if full_minimum_different:
        print('If the minimum and full promoters have different paramterisations, then we need to consider both')
        print('E.g., MAKE promoter_selection == both')
        print('Try again!')
        sys.exit()
# ----------------------------------------------------------------------------------------------------------------------
# Models to use
...
# PleuWTmin
if full_minimum_different:
    variation_dict['PleuWTmin'] = variation
...
if full_minimum_different:
    # PleuWT
    space['PleuWTmin_k_on'] =  hp.uniform('PleuWTmin_k_on', k_on_min, k_on_max)
    space['PleuWTmin_spread'] =  hp.uniform('PleuWTmin_spread', spread_min, spread_max)
    space['PleuWTmin_superhelical_op'] =  hp.uniform('PleuWTmin_superhelical_op', superhelical_op_min, superhelical_op_max)
    space['PleuWTmin_k_closed'] =  hp.uniform('PleuWTmin_k_closed', k_closed_min, k_closed_max)
    space['PleuWTmin_k_open'] =  hp.uniform('PleuWTmin_k_open', k_open_min, k_open_max)
    space['PleuWTmin_k_ini'] =  hp.uniform('PleuWTmin_k_ini', k_ini_min, k_ini_max)
    space['PleuWTmin_k_off'] =  hp.uniform('PleuWTmin_k_off', k_off_min, k_off_max)
...
def objective_function(params, calibrating=True):
    ...
    if full_minimum_different:
        PleuWTmin_variation = variation_dict['PleuWTmin']
        PleuWTmin_variation['binding_oparams']['k_on'] = params['PleuWTmin_k_on']
        PleuWTmin_variation['binding_oparams']['k_off'] = params['PleuWTmin_k_off']
        PleuWTmin_variation['binding_oparams']['k_open'] = params['PleuWTmin_k_open']
        PleuWTmin_variation['binding_oparams']['k_closed'] = params['PleuWTmin_k_closed']
        PleuWTmin_variation['binding_oparams']['k_ini'] = params['PleuWTmin_k_ini']
        PleuWTmin_variation['binding_oparams']['superhelical_op'] = params['PleuWTmin_superhelical_op']
        PleuWTmin_variation['binding_oparams']['spread'] = params['PleuWTmin_spread']
    else:
        PleuWTmin_variation = PleuWT_variation
    ...
    my_variation = [PleuWTmin_variation, tetA_variation, antitet_variation, bla_variation,
